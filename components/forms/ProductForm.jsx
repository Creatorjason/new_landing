...
const ProductForm = ({ setIsProductSuccess, verseId }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    submitFormData.append('quantity', parseInt(formData.quantity, 10)); // Ensure quantity is an integer
    if (formData.file) {
    ...
    try {
      const response = await axios.post(
        `https://api.granularx.com/verse/products/add/${verseId}`, 
        submitFormData,
        {
          headers: {
